...
export function SectionHeading({ title, subtitle, centered = true, className }: Props) {
  return (
    <div className={cn('mb-12 lg:mb-16', centered && 'text-center', className)}>
      ...
      <motion.div
        className={cn('h-[3px] bg-gradient-to-r from-teal to-teal-deep rounded-full', centered && 'mx-auto')}
      ...
      {subtitle && (
        <SlideIn from="bottom" delay={0.2}>
          <p className="mt-5 font-body text-body-lg text-body max-w-2xl mx-auto leading-relaxed">{subtitle}</p>
        </SlideIn>
      )}
    </div>
  )
